...
Manuscript is currently in minor revision at JMIR Medical Informatics. This repository release (2.2.0) corresponds to the revised submission dated 10 July 2026. Changes since the initial submission include a corrected per-query mixed-effects analysis, updated anisotropy tier boundaries, reconciliation of the final-layer baseline MRR@10 across pipelines, and the deposit of the complete revision-era pipeline scripts (Streams A, B, C, E, G) — see docs/analysis_decisions.md for the full record.
...
│   └── revision/                        # Revision-era additions (v17 → final)
│       ├── stream_a_matched_comparisons.py       # Matched controls: BERT-base + BioMistral
│       ├── stream_b_zca_methodology.py           # ZCA methodology + ε sweep + 5-fold CV
│       ├── stream_c_per_query_ranks_lme.py       # Per-query ranks for LME + first-pass fits
│       ├── stream_eg_editorial_analyses.py       # Lexical overlap, synthetic audit, geometry,
│       │                                         #   MedCPT expanded, E5 sweep, chunking
│       ├── revision_figures.py                   # All revision figures from cached parquets
│       ├── colab_validation_extension.py         # 5-model validation on 500 PMC + 400 MTSamples
│       ├── prepare_500doc_corpora.py             # GPT-4o queries for validation extension
...
│   └── (per-model stream A/B/C/G output parquets)
│
...
│
└── docs/
├── analysis_decisions.md            # Corrected LME, tier reconciliation, baseline reconciliation
...
- PMC-Patients: 100 documents sampled from zhengyun21/PMC-Patients with seed=42
...
Set `HF_TOKEN` as an environment variable before running any script that accesses gated models (BioMistral-7B, E5-Mistral-7B, some Phi-3 variants):

```bash
export HF_TOKEN=your_hf_token_here
```

Embedding extraction tested on Google Colab Pro+ with NVIDIA H100 80GB and NVIDIA RTX PRO 6000 Blackwell 102 GB. Analytical scripts run on CPU.

### Three levels of reproduction

**(A)** Verify manuscript numbers against published outputs (minutes, no GPU). Every manuscript table and figure value is derivable from a file in `results/` or `supplementary/`. See `docs/reproducibility.md` for the claim-to-file mapping.
...
**(C)** Full revision-era pipeline from raw text (~6–8 hours, GPU + CPU).

The revision-era analytical Streams A, B, C, E, and G are fully deposited in `scripts/revision/` as Colab-compatible Python scripts. Run order:

```bash
# 1. Matched-control extraction (BERT-base-uncased + BioMistral-7B)
python scripts/revision/stream_a_matched_comparisons.py

# 2. ZCA methodology + ε sensitivity + 5-fold CV, all 13 configurations
python scripts/revision/stream_b_zca_methodology.py

# 3. Per-query per-layer ranks for LME input
python scripts/revision/stream_c_per_query_ranks_lme.py

# 4. Corrected LME re-fit
python scripts/revision/v1_final_refit.py --input per_query_ranks.parquet --output-dir results/

# 5. Combined editorial-response analyses (lexical overlap, synthetic audit,
#    MedCPT expanded, geometry, E5 intervention sweep, chunking sensitivity)
python scripts/revision/stream_eg_editorial_analyses.py

# 6. Extended validation on 500 PMC-Patients + 400 MTSamples
python scripts/revision/prepare_500doc_corpora.py
python scripts/revision/colab_validation_extension.py

# 7. Figures from cached parquets
python scripts/revision/revision_figures.py
```

**(D)** Initial-pipeline reproduction from raw text (~3 hours, GPU).
...
Archived: [10.5281/zenodo.21292667](https://doi.org/10.5281/zenodo.21292667)
